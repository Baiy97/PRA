...
'''
@register_op
class OverSample(BaseOperator):
    def __init__(self):
        self.patch_img_dir = 'dataset/camera/box_eyes'
        self.patch_gt = 'dataset/camera/box_eye.json'
        with open(self.patch_gt, 'r') as f:
            self.patchs = json.load(f)
        self.num_try = 10

    def __call__(self, sample, cate_id=3):
        if isinstance(sample, Sequence) or 1 not in sample['Aug']:
            return sample

        patch_items = self.patchs[str(cate_id)]
        img = sample['image']
        H, W = img.shape[:2]
        # get patch
        patch_index = random.randint(0, len(patch_items)-1)
        patch_item = patch_items[patch_index]
        with open(os.path.join(self.patch_img_dir, patch_item['image']), 'rb') as f:
            patch_data = f.read()
        patch_data = np.frombuffer(patch_data, dtype='uint8')
        patch = cv2.imdecode(patch_data, 1)  # BGR mode, but need RGB mode
        patch = cv2.cvtColor(patch, cv2.COLOR_BGR2RGB)
        h, w = patch.shape[:2]
        polygon, box = patch_item['polygon'], patch_item['box']
        polygon = np.array(polygon).reshape(-1, 2)
        box = np.array(box).reshape(1, 4)
        # do transform to the patch
        # only resize here, could add more transform operations
        h_scale = np.random.uniform(0.5, 1.5)
        w_scale = h_scale * np.random.uniform(0.8, 1.2)
        patch = cv2.resize(patch, (int(w * w_scale), int(h * h_scale)))
        h, w = patch.shape[:2]
        box[:, :2] = box[:, :2] * (w_scale, h_scale)
        box[:, 2:] = box[:, 2:] * (w_scale, h_scale)
        polygon = polygon * (w_scale, h_scale)

        find = False
        # choose a location
        start_x = random.randint(0, W-1-w)
        if h * w < 40 * 40:
            start_y = random.randint(0, H//3-1-h)
        elif h * w < 100 * 100:
            start_y = random.randint(0, H//2-1-h)
        else:
            start_y = random.randint(0, H-1-h)
        
        for gt_bbox_i in sample['gt_bbox']:
            if start_x + w < gt_bbox_i[0] and start_y + h < gt_bbox_i[1]:
                find = True
                break
            elif start_x > gt_bbox_i[0] + gt_bbox_i[2] and start_y + h < gt_bbox_i[1]:
                find = True
                break
            elif start_x + w < gt_bbox_i[0] and start_y > gt_bbox_i[1] + gt_bbox_i[3]:
                find = True
                break
            elif start_x > gt_bbox_i[0] + gt_bbox_i[2] and start_y > gt_bbox_i[1] + gt_bbox_i[3]:
                find = True
                break
        ...
        if find:
            ...
            # apply image
            mask = np.zeros((patch.shape[0], patch.shape[1], patch.shape[2]), dtype=np.uint8)
            cv2.fillPoly(mask, polygon.reshape(1, -1, 2).astype(np.int32), (1, 1, 1))
            img[start_y:start_y+h, start_x:start_x+w] = mask * patch + (1 - mask) * img[start_y:start_y+h, start_x:start_x+w]
            result['image'] = img
            box[:, :2] = box[:, :2] + (start_x, start_y)
            # apply anno
            gt_bbox = sample['gt_bbox']
            gt_bbox = np.concatenate((gt_bbox, box), axis=0)
            result['gt_bbox'] = gt_bbox
            if 'gt_class' in sample:
                gt_class = np.concatenate((sample['gt_class'], cate_id * np.ones((1, 1), dtype=np.int32)), axis=0)
                result['gt_class'] = gt_class
                gt_score = np.concatenate((np.ones_like(sample['gt_class']), np.ones((1, 1), dtype=np.int32)), axis=0)
                result['gt_score'] = gt_score
            if 'is_crowd' in sample:
                is_crowd = np.concatenate((sample['is_crowd'], np.zeros((1, 1), dtype=np.int32)), axis=0)
                result['is_crowd'] = is_crowd
            if 'difficult' in sample:
                is_difficult = np.concatenate((sample[0]['difficult'], np.zeros((1, 1), dtype=np.int32)), axis=0)
...
@register_op
class PyramidImage(BaseOperator):
    def __init__(self, scale_ratio=0.5):
        '''Paste image to make pyramid
        this is a simple implementation, only paste the image if background area around the eimage is enough
        Args:
            resize_ratio (float): resize ratio for image
        '''
        self.scale_ratio = scale_ratio

    def __call__(self, sample):
        if isinstance(sample, Sequence) or 2 not in sample['Aug']:
            return sample
        result = copy.deepcopy(sample)
        image = sample['image']
        H, W = image.shape[:2]
        gt_bbox = sample['gt_bbox']
        x_min, y_min = min(gt_bbox[:, 0]), min(gt_bbox[:, 1])
        x_max = max([t[0] + t[2] for t in gt_bbox])
        y_max = max([t[1] + t[3] for t in gt_bbox])
        h, w = int(H * self.scale_ratio), int(W * self.scale_ratio)
        image_ = cv2.resize(image, (w, h))
        # paste location: corners of left-top, right-top, left-bottom, right-bottom
        # corners = [[1, 0, 0, w, h], [2, W-w, 0, W, h], [3, 0, H-h, w, H], [4, W-w, H-h, W, H]]
        corners = [1, 2, 3, 4]
        random.shuffle(corners)
        do_apply = False
        for corner in corners:  # search on the corners
            if corner == 1 and w < x_min and h < y_min:
                image[:h, :w, :] = image_
                # apply image
                result['image'] = image
                # apply gt_bbox
                gt_bbox = sample['gt_bbox']
                gt_bbox = gt_bbox * self.scale_ratio
                result['gt_bbox'] = gt_bbox
                do_apply = True
                break
            elif corner == 2 and W-w > x_max and h < y_min:
                image[:h, W-w:W, :] = image_
                result['image'] = image
                gt_bbox = sample['gt_bbox']
                gt_bbox = gt_bbox * self.scale_ratio
                gt_bbox[:, :2] = gt_bbox[:, :2] + (W-w, 0)
                result['gt_bbox'] = gt_bbox
                do_apply = True
                break
            elif corner == 3 and w < x_min and H-h > y_max:
                image[H-h:H, :w, :] = image_
                result['image'] = image
                gt_bbox = sample['gt_bbox']
                gt_bbox = gt_bbox * self.scale_ratio
                gt_bbox[:, :2] = gt_bbox[:, :2] + (0, H-h)
                result['gt_bbox'] = gt_bbox
                do_apply = True
                break
            elif corner == 4 and W-w > x_max and H-h > y_max:
                image[H-h:H, W-w:W, :] = image_
                result['image'] = image
                gt_bbox = sample['gt_bbox']
                gt_bbox = gt_bbox * self.scale_ratio
                gt_bbox[:, :2] = gt_bbox[:, :2] + (W-w, H-h)
                result['gt_bbox'] = gt_bbox
                do_apply = True
                break
        ...
        if do_apply:
            ...
            if 'gt_class' in sample:
                gt_class = sample['gt_class']
                gt_class = np.concatenate((gt_class, copy.deepcopy(gt_class)), axis=0)
                result['gt_class'] = gt_class
                gt_score = np.ones_like(sample['gt_class'])
                gt_score = np.concatenate((gt_score, copy.deepcopy(gt_score)), axis=0)
                result['gt_score'] = gt_score
            '''
            if is_crowd or difficult is related to box area, modify the code
            '''
            if 'is_crowd' in sample:
                is_crowd = sample['is_crowd']
                is_crowd = np.concatenate((is_crowd, copy.deepcopy(is_crowd)), axis=0)
                result['is_crowd'] = is_crowd
            if 'difficult' in sample:
                is_difficult = sample['difficult']
                is_difficult = np.concatenate((is_difficult, copy.deepcopy(is_difficult)), axis=0)
                result['difficult'] = is_difficult
...
class Mosaic(BaseOperator):
    def __init__(self):
        '''Mosaic image and anno
        if the sizes of image are different, choose the largest image as a quarter or resize images.
        '''

    def apply_2sample(self, sample, direction=0):
        '''
        direction 1 means horizontal concat, 0 means vertical concat
        '''
        result = copy.deepcopy(sample[0])
        img1, img2 = [sample[i]['image'] for i in range(2)]
        h, w = img1.shape[:2]
        if h != img2.shape[0] or w != img2.shape[1]:
            h_, w_ = img2.shape[:2]
            img2 = cv2.resize(img2, (w, h))
            sample[1]['gt_bbox'] = sample[1]['gt_bbox'] * (w / w_, h / h_, w / w_, h / h_)

        if direction == 0:
            # apply image
            image = np.zeros((2 * h, w, img1.shape[2]), 'uint8')
            image[:h, :, :] = img1
            image[h:, :, :] = img2
            result['image'] = image
            # apply gt_bbox
            if 'gt_bbox' in sample[0]:
                gt_bbox1 = sample[0]['gt_bbox']
                gt_bbox2 = sample[1]['gt_bbox']
                gt_bbox2[:, 1] = gt_bbox2[:, 1] + h
                gt_bbox = np.concatenate((gt_bbox1, gt_bbox2), axis=0)
                result['gt_bbox'] = gt_bbox
        else:
            # apply image
            image = np.zeros((h, 2 * w, img1.shape[2]), 'uint8')
            image[:, :w, :] = img1
            image[:, w:, :] = img2
            result['image'] = image
            # apply gt_bbox
    ...
    def apply_4sample(self, sample):
        result = copy.deepcopy(sample[0])
        img1, img2, img3, img4 = [sample[i]['image'] for i in range(4)]
        h, w = img1.shape[:2]
        # resize other images
        if h != img2.shape[0] or w != img2.shape[1]:
            h_, w_ = img2.shape[:2]
            img2 = cv2.resize(img2, (w, h))
            sample[1]['gt_bbox'] = sample[1]['gt_bbox'] * (w / w_, h / h_, w / w_, h / h_)
        if h != img3.shape[0] or w != img3.shape[1]:
            h_, w_ = img3.shape[:2]
            img3 = cv2.resize(img3, (w, h))
            sample[2]['gt_bbox'] = sample[2]['gt_bbox'] * (w / w_, h / h_, w / w_, h / h_)
        if h != img4.shape[0] or w != img4.shape[1]:
            h_, w_ = img4.shape[:2]
            img4 = cv2.resize(img4, (w, h))
            sample[3]['gt_bbox'] = sample[3]['gt_bbox'] * (w / w_, h / h_, w / w_, h / h_)
            
        # apply image
        image = np.zeros((2 * h, 2 * w, img1.shape[2]), 'uint8')
        image[:h, :w, :] = img1
        image[:h, w:, :] = img2
        image[h:, :w, :] = img3
        image[h:, w:, :] = img4
        ...
        if 'gt_bbox' in sample[0]:
            gt_bbox1 = sample[0]['gt_bbox']
            gt_bbox2 = sample[1]['gt_bbox']
            gt_bbox3 = sample[2]['gt_bbox']
            gt_bbox4 = sample[3]['gt_bbox']
            gt_bbox2[:, :2] = gt_bbox2[:, :2] + (w, 0)
            gt_bbox3[:, :2] = gt_bbox3[:, :2] + (0, h)
            gt_bbox4[:, :2] = gt_bbox4[:, :2] + (w, h)
            gt_bbox = np.concatenate((gt_bbox1, gt_bbox2, gt_bbox3, gt_bbox4), axis=0)
            result['gt_bbox'] = gt_bbox
        if 'gt_class' in sample[0]:
            gt_classes = [sample[i]['gt_class'] for i in range(4)]
            gt_class = np.concatenate(gt_classes, axis=0)
            result['gt_class'] = gt_class
            gt_scores = [np.ones_like(sample[i]['gt_class']) for i in range(4)]
            gt_score = np.concatenate(gt_scores, axis=0)
            result['gt_score'] = gt_score
        '''
        if is_crowd or difficult is related to box area, modify the code
        '''
        if 'is_crowd' in sample[0]:
            is_crowds = [sample[i]['is_crowd'] for i in range(4)]
            is_crowd = np.concatenate(is_crowds, axis=0)
            result['is_crowd'] = is_crowd
        if 'difficult' in sample[0]:
            is_difficults = [sample[i]['difficult'] for i in range(4)]
            is_difficult = np.concatenate(is_difficults, axis=0)
            result['difficult'] = is_difficult

        return result

    def __call__(self, sample):
        if not isinstance(sample, Sequence) or 4 not in sample[0]['Aug']:
            return sample

        # assert len(sample) == 4, 'mosaic need four samples'
        if len(sample) == 2:
            if random.random() < 0.5:
                return self.apply_2sample(sample, direction=1)
            else:
                return self.apply_2sample(sample, direction=0)
        elif len(sample) == 4:
            return self.apply_4sample(sample)
        else:
            raise ValueError('Num of samples should be 2 or 4')
...
class Mixup(BaseOperator):
    ...
    def __call__(self, sample, context=None):
        if not isinstance(sample, Sequence) or 3 not in sample[0]['Aug']:
            return sample
